...
class RWKV7FeedForward(nn.Module):
    ...
    def _full_forward(self, hidden_states: torch.Tensor, output: torch.Tensor) -> None:
        ...
        if is_spec:
            # Per-step FFN-shift writes for spec sequences (pure-spec batches).
            flat_slots = m.spec_state_indices_tensor.reshape(-1)
            valid_mask = flat_slots >= 0
            if valid_mask.any():
                valid_slots = flat_slots[valid_mask]
                ffn_shift_cache[valid_slots, 0] = x[valid_mask].to(
                    ffn_shift_cache.dtype
                )
        else:
            update_shift_state(
                hidden_states=x,
