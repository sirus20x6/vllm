...
class MambaStateShapeCalculator:
    ...
    def rwkv7_state_shape(
        ...
        attn_shift_shape = (1 + num_spec, divide(hidden_size, tp_world_size))
        ffn_shift_shape = (1 + num_spec, divide(hidden_size, tp_world_size))
...
class MambaStateCopyFuncCalculator:
    ...
    def rwkv7_state_copy_func(cls):
        # RWKV-7 shift states are single-row hidden_states for the previous
        # token, not a multi-token conv buffer. Use temporal copy semantics
        # (whole state at the accepted-token offset) for both shift caches.
        return (
            get_temporal_copy_spec,
            get_temporal_copy_spec,
            get_temporal_copy_spec,
