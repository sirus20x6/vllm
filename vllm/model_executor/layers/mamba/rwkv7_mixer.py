...
def resolve_spec_read_slot(
    spec_state_indices_tensor: torch.Tensor,
    num_accepted_tokens: torch.Tensor,
) -> torch.Tensor:
    """Slot per spec sequence holding the state at the last accepted token.

    ``spec_state_indices_tensor`` has shape ``[N_spec, num_spec + 1]``;
    ``num_accepted_tokens`` is shape ``[N_spec]``. Returns the 1D ``[N_spec]``
    cache-slot ids to read the initial shift / recurrent state from for the
    next spec round.
    """
    cols = (num_accepted_tokens.long() - 1).clamp(min=0).unsqueeze(1)
    return spec_state_indices_tensor.gather(1, cols).squeeze(1)
...
class RWKV7Attention(nn.Module, MambaBase):
    ...
    def _full_forward(
        ...
        is_spec = num_spec_decodes > 0
        if is_spec:
            # Pure-spec batches only for v1. The metadata builder reclassifies
            # any plain decodes as 1-token prefills when spec is active, so we
            # only need to assert there are no real prefills.
            assert num_prefills == 0, (
                "RWKV-7 spec decoding does not yet support mixed prefill+spec "
                "batches"
            )
        ...
        if is_spec:
            # Spec sequences: read from the accepted-position slot.
            read_slot = resolve_spec_read_slot(
                m.spec_state_indices_tensor, m.num_accepted_tokens
            )
            # write_slot isn't used in spec mode (kernel does per-step writes)
            # but compute_token_shift_delta needs a 1D tensor.
            write_slot = read_slot
        elif m.read_slot is None or m.write_slot is None:
        ...
        if is_spec:
            # Spec path: kernel reads initial state from
            #   recurrent_state_cache[spec_state_indices[i, num_accepted-1]]
            # and writes per-step states to
            #   recurrent_state_cache[spec_state_indices[i, t]]
            # So no Python-side gather/scatter needed for the recurrent state.
            initial_state = None
        else:
        ...
        # Dispatch:
        #  - spec batch: use the recurrent kernel with ssm_state_indices to
        #    write per-step recurrent states into the spec cache slots.
        ...
        if is_spec:
            spec_state_indices = m.spec_state_indices_tensor.to(torch.int32)
            num_accepted = m.num_accepted_tokens.to(torch.int32)
            core_out, _final_state = fused_mul_recurrent_rwkv7(
                r=r_4d,
                w=w_4d,
                k=k_4d,
                v=v_4d,
                ...
                ht=recurrent_state_cache,
            )
            final_state = None  # already written per-step by the kernel
            varlen_states = None
        elif num_prefills == 0:
        ...
        if not is_spec:
            ...
            recurrent_state_cache[write_slot] = final_state.to(
                recurrent_state_cache.dtype
            )
        # In spec mode the kernel already scattered per-step recurrent states
        # into spec_state_indices_tensor.
        ...
        if is_spec:
            # Per-step shift state writes for spec sequences. Each token t of
            # spec sequence i gets its post-attn-norm hidden state stored to
            # the slot at spec_state_indices_tensor[i, t]. Pure-spec batches
            # only (asserted above), so x rows align 1:1 with the flattened
            # spec table.
            flat_slots = m.spec_state_indices_tensor.reshape(-1)
            valid_mask = flat_slots >= 0
            if valid_mask.any():
                valid_slots = flat_slots[valid_mask]
                attn_shift_cache[valid_slots, 0] = x[valid_mask].to(
                    attn_shift_cache.dtype
                )
        else:
            update_shift_state(
                hidden_states=x,
